...
void InitSpi(void)
{
    ...
    // 16-bit character
    // Enable loop-back
    // SPILBK (loopback mode 0 == disabled 1 == enabled. When eabled, SIMO/SOMI lines are connected internally.)
    SpiaRegs.SPICCR.bit.SPISWRESET = 0;
    SpiaRegs.SPICCR.bit.CLKPOLARITY = 0;
    SpiaRegs.SPICCR.bit.SPICHAR = (9-1);
...
//  GpioCtrlRegs.GPAPUD.bit.GPIO3 = 0;   // Enable pull-up on GPIO3 (SPISOMIA)
    ...
    //
    // This will select asynch (no qualifica tion) for the selected pins.
    // Comment out other unwanted lines.
    //
